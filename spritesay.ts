namespace sprites_ {
    export class BaseSpriteSayRenderer {
        ...
        draw(screen: Image, camera: scene.Camera, owner: Sprite_) {

        }

        update(dt: number, camera: scene.Camera, owner: Sprite_) {
    ...
    export class SpriteSayRenderer extends BaseSpriteSayRenderer {
        ...
        constructor(text: string, fg: number, bg: number, animated: boolean, timeOnScreen: number) {
            ...
            if (animated) {
                this.animation = new RenderTextAnimation(this.renderText, 40);
                if (timeOnScreen >= 0) {
        ...
        draw(screen: Image, camera: scene.Camera, owner: Sprite_) {
            ...
            SpriteSayRenderer.drawSayFrame(sayLeft, sayTop, width, height, owner._x, owner._y, this.bgColor, screen);
    ...
    export class LegacySpriteSayRenderer extends BaseSpriteSayRenderer {
        ...
        update(dt: number, camera: scene.Camera, owner: Sprite_) {
            ...
            this.sayBubbleSprite.setFlag(SpriteFlag.RelativeToCamera, !!false);
        }
