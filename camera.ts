...
export class Camera_ {
   // coordinate used for all physics computation
    ...
    isUpdated() {
        return !this.sprite || (this.sprite._x === this._lastUpdatedSpriteX && this.sprite._y === this._lastUpdatedSpriteY);
    }

    update() {
        // if sprite, follow sprite
        if (this.sprite) {
            this._lastUpdatedSpriteX = this.sprite._x;
            this._lastUpdatedSpriteY = this.sprite._y;
            this.offsetX = this.sprite._x - (screen.width >> 1);
            this.offsetY = this.sprite._y - (screen.height >> 1);
        }
